...
class DashboardTab(QWidget):
    ...
    def __init__(self):
        ...
        # --- Fila 1: Uptime ---
        row1 = QHBoxLayout()
        self.card_uptime30 = KPICard("Uptime 30 días", "—")
        self.card_uptime7  = KPICard("Uptime 7 días",  "—")
        ...
        # --- Fila 2: Fallos ---
        row2 = QHBoxLayout()
        self.card_mtbf = KPICard("MTBF (30 d)", "—", "tiempo medio entre fallos")
        self.card_mttr = KPICard("MTTR (30 d)", "—", "tiempo medio de recuperación")
        self.card_cuts = KPICard("Cortes eléctricos (30 d)", "—")
        ...
        self.card_net_downs  = KPICard("Caídas de red (30 d)", "—")
        ...
        try:
            ...
            if net["ok"]:
                self.card_net_status.set(
                    "● En línea",
                    f"último ping: {net['last_ping_ms']:.0f} ms"
                    if net["last_ping_ms"] is not None else "sin latencia",
                )
                self.card_net_status.set_value_color("#2E7D32")
            else:
                self.card_net_status.set(
                    "● Sin conexión",
                    f"último intento: {net['last_time']}",
                )
            ...
            import logging
            logging.warning(f"Dashboard refresh falló: {e}")